...
def run_trials(data, station, config, *, method, save_root, prepare_s, prompt=None):
    ...
    with ExitStack() as stack:
        ...
        recorder = EpisodeRecorder(
            ...
            native_sources_factory=backend.driver.native_joint_sources,
        )
        ...
        for selected in methods:
            ...
            try:
                ...
                episode = recorder.start(f"command_replay_{selected}", record_native_joints=True)
...
def main(argv=None):
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/collection/yam/station.yaml"))
    parser.add_argument("--cameras", type=Path, default=Path("configs/collection/yam/cameras.yaml"))
