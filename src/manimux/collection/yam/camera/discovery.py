"""Enumerate connected cameras (RealSense + Orbbec + V4L2/UVC decxin) with serials.
...
def discover_v4l2() -> list[dict]:
    ...
    try:
        ...
        return []
    from .orbbec import discover_orbbec

    by_serial: dict[str, dict] = {}
    for dev in pyudev.Context().list_devices(subsystem="video4linux"):
        # Orbbec has depth/IR/metadata nodes as well as RGB; its dedicated
        # discovery selects the primary color interface instead of the first node.
    ...
    return discover_orbbec() + sorted(by_serial.values(), key=lambda d: d["node"])
