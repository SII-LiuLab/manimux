...
function renderRail(cfg) {
  ...
  // These controls are created by renderRail(), so attach their handlers only
  // after the elements exist. The teaching-handle record button uses backend
  // options, therefore path/task edits are synced while the user types.
  $("save-root").oninput = scheduleRecordingOptionsSync;
  $("task").oninput = scheduleRecordingOptionsSync;
  $("save-root").onchange = syncRecordingOptions;
  $("fmt").onchange = syncRecordingOptions;
  $("task").onchange = syncRecordingOptions;
}
...
// own buttons/trigger, follower joints, upstream action, and MIT controller input.
let armSig = "";
...
function armVecLen(arms) {
  for (const a of Object.values(arms || {})) {
    const v = a.follower || a.leader_raw || a.leader_cmd || a.controller_cmd;
...
function armCard(name, nJoints) {
  ...
  return `<div class="armcard" data-arm="${name}">
    ...
    <div class="jgrid" style="${cols}">
      ...
      <span class="rl act" title="joint target input to the MIT controller">MIT</span>${cells("mit")}<span class="n g" data-mit="g">—</span>
    </div>
  </div>`;
}
...
function renderArms(s) {
  ...
  for (const u of units) {
    ...
    // Controller diagnostics intentionally contain arm joints only; GRIP stays blank.
    setArmRow(card, "mit", a.controller_cmd, false, false);
...
function connectWS() {
  ...
  ws.onmessage = (ev) => {
    ...
    const savingEpisode = !!s.saving_episode;
    $("rec-label").textContent = savingEpisode
      ? (s.record_eepose ? "Computing EE Pose + Saving…" : "Saving Episode…")
    ...
    $("rec-lamp-txt").textContent = savingEpisode
      ? (s.record_eepose ? "EE pose + saving" : "saving")
      : (rec ? "recording" : "idle");
    const activeRecordPath = s.record_path || s.last_episode_path || "";
    $("rec-lamp").title = activeRecordPath ? `episode: ${activeRecordPath}` : "";
    if (!rec && !savingEpisode && s.last_episode_path &&
        s.last_episode_path !== _lastSavedEpisodeShown) {
      _lastSavedEpisodeShown = s.last_episode_path;
      toast(`Episode saved: ${s.last_episode_path}`, s.last_record_warning ? "warn" : "ok");
      if (s.last_record_warning) toast(s.last_record_warning, "warn");
    }
    if (s.record_task_required &&
        s.record_task_request_id !== _lastRecordTaskRequest) {
      _lastRecordTaskRequest = s.record_task_request_id;
      alert("Set a Task name first — recording was not started.\n" +
            "The task becomes the episode folder and the training prompt.");
      $("task").focus();
    }
...
$("btn-rec").onclick = async () => {
  ...
  if (recording) {
    const result = await post("/api/collect/stop-recording");
  ...
  if (result.detail) toast(`Start Recording refused: ${result.detail}`, "err");
  else toast(`Saving to ${result.episode}`, "ok");
};
$("record-eepose").onchange = async () => {
...
const syncRecordingOptions = async () => {
  ...
  if (result.detail) toast(`Recording settings refused: ${result.detail}`, "warn");
};
// The physical second button starts recording in the backend, so it cannot read an
// unsent browser input. Push path/task edits shortly after typing instead of waiting
// for blur/change (pressing a physical button does not blur the input).
const scheduleRecordingOptionsSync = () => {
  clearTimeout(_recordOptionsTimer);
  _recordOptionsTimer = setTimeout(syncRecordingOptions, 120);
};
...
$("dp-backend").addEventListener("change", () => {
  ...
  // Pi RTC needs gradient access to the flow denoiser. ABC and MolmoAct2
  // advertise that capability; autoregressive/openpi backends do not.
  const rtcCapable = ["abc", "molmoact2"].includes(b);
  $("dp-rtc").disabled = !rtcCapable;
  if (!rtcCapable) $("dp-rtc").checked = false;
