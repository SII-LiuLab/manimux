...
from manimux.embodiments.arm.tianji.kinematics import rotation_matrix, rotation_vector
...
class XR1TianjiTacCapAdapter(PolicyAdapterBase):
    ...
    def __init__(self, robot: dict, policy: dict, *, kinematics=None) -> None:
        ...
        self.policy = policy
        self._group_order = tuple(policy["options"].get("group_order", GROUP_DIMS))
        self._camera_map = dict(policy["options"].get("camera_map", {}))
    ...
    def _solve_knot(self, model, current, target, gripper):
        ...
        step_dt_s = float(self.policy["options"].get("ik_validation_dt_s", 0.004))
        if not np.isfinite(step_dt_s) or step_dt_s <= 0:
            raise ValueError("policy.options.ik_validation_dt_s must be finite and positive")
        backend_limit = model.arm.max_step_duration_s
        ...
        for index in range(1, substeps + 1):
            ...
            result = model.ik(
                ...
                duration_s=self._action_dt_s / substeps,
            )
            if not result.ok:
