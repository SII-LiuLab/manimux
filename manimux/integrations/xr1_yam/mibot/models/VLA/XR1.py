...
from manimux.integrations.xr1_yam.mibot.models.VLM.qwen3vl import (
    ACTION_END_ID,
    ACTION_START_ID,
    SCORE_ID,
    STATE_ID,
...
from manimux.integrations.xr1_yam.mibot.utils.model_utils import auto_cast
...
# Upstream hardcodes the Qwen3-VL repo id, which forces a Hugging Face lookup at
# construction time. Only `config.json` is read (`_from_config` random-initializes
# and every VLM weight then comes from the checkpoint), so point this at a local
# directory to build the model fully offline.
...
class xr1(nn.Module):
    ...
    def _generate(self, noise, kwargs):
        # patched_rtc: with a real-time-chunking condition attached, sample with
        # PiGDM guidance instead. Without one this is upstream's loop, unchanged.
    ...
    # ---- patched_rtc: real-time chunking (arXiv:2506.07339) -----------------
    #
    # XR-1 integrates flow time 0 -> 1 with `sample += v * dt`, the same
    # convention as the paper, so this is a direct transcription of Algorithm 1
    # lines 27-30 with no sign flip:
    #
    #     f(A)  = A + (1 - t) * v(A, t)                      (Eq. 3)
    #     e     = (Y - f(A)) * W                             weighted error
    #     g     = e @ df/dA                                  VJP
    #     A    += dt * (v + min(beta, (1-t)/(t r_t^2)) * g)  (Eq. 2)

    @contextlib.contextmanager
    def rtc_condition(self, condition, weights, beta: float = 5.0):
        """Attach an inpainting condition for the duration of one generate call.

        ``condition`` is ``(B, H, D)`` in the model's *normalized* action space
        and ``weights`` is ``(B, H)`` or ``(B, H, 1)``. Guidance is per-request
        state on a shared model, so exactly one may be active at a time.
        """
        if getattr(self, "_rtc_condition", None) is not None:
    ...
    def _rtc_guidance_scale(self, flow_time: float) -> float:
        ...
        beta = self._rtc_beta
        if flow_time <= 0.0:
            return beta  # the exact expression diverges at t = 0
        if flow_time >= 1.0:
            return 0.0
        tau = flow_time
        return min(beta, (tau**2 + (1.0 - tau) ** 2) / (tau * (1.0 - tau)))
    ...
    def _generate_pi_rtc(self, noise, kwargs):
        ...
        sample = noise.clone()
        dt = 1.0 / self.num_steps
        # The VLM prefix and its KV cache stay out of the graph; only the DiT's
        # action path is differentiated, and only w.r.t. the noisy chunk.
        with torch.enable_grad():
            for step in range(self.num_steps):
                flow_time = step / self.num_steps
                x_t = sample.detach().requires_grad_(True)
                timestep = torch.ones(
                    (x_t.shape[0], 1, 1), device=x_t.device, dtype=x_t.dtype
                ) * flow_time
                velocity = self.dit_forward(x_t, timestep, **kwargs)
                clean_estimate = x_t + (1.0 - flow_time) * velocity
                error = ((target - clean_estimate) * weights).detach()
                correction = torch.autograd.grad(
                    clean_estimate, x_t, grad_outputs=error, create_graph=False
                )[0]
                guided = velocity + self._rtc_guidance_scale(flow_time) * correction
                sample = (x_t + dt * guided).detach()
        return sample
