...
from .molmoact_rtc import MolmoActRTC
...
class Policy:
    ...
    def __init__(
        ...
        # RTC differentiates only with respect to the sampled action trajectory.
        # Freezing parameters avoids building unnecessary parameter gradients.
        ...
        self._rtc = MolmoActRTC(self.model, NORM_TAG)
    ...
    def predict(
        ...
        with (
            self._lock,
            self._rtc.condition(action_condition, condition_weights, rtc_beta) as rtc_active,
        ):
            out = self.model.predict_action(
                ...
                # CUDA graphs cannot capture the RTC vector-Jacobian product.
                enable_cuda_graph=enable_cuda_graph and not rtc_active,
            )
        raw = out.actions
...
def build_app(policy: Policy) -> FastAPI:
    ...
    async def act(request: Request) -> Response:
        ...
        enable_cuda_graph = bool(payload.get("enable_cuda_graph", policy.default_cuda_graph))
...
def warmup(policy: Policy) -> None:
    log.info("Warming up model with dummy frames (cuda_graph=%s) ...", policy.default_cuda_graph)
