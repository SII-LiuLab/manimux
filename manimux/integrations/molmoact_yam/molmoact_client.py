...
class MolmoAct(PolicyBase):
    def __init__(self, server: str | None = None):
        ...
        # Keep transport/release phases coherent while still replanning often.
        # The server returns 30 actions; the async loop consumes a 20-step
        # receding-horizon window and retains at least one full window when
        # accounting for inference staleness.
    ...
    def prepare_input(self, obs, instruction):
        ...
        # Camera frames are intentionally not logged; they are large arrays.
    ...
    def send_request(
        self, images: list[np.ndarray], instruction: str, state: list, server_url: str
    ):
        """
        ...
        try:
            if not self.multi_views:
                ...
                payload = {
                    ...
                    # Cuts the steady-state action-expert latency roughly in half
                    # on this RTX 4090 host after the first graph capture.
...
class _LocalPolicy:
    ...
    def predict(
        ...
        if state_f32.shape != (STATE_DIM,):
            raise ValueError(f"state must be shape ({STATE_DIM},), got {state_f32.shape}")
...
class MolmoActLocal(PolicyBase):
    ...
    def __init__(
        ...
        self.logger.info(f"MolmoActLocal: loading {repo_id} (device={device}, dtype={dtype})")
    ...
    def prepare_input(self, obs: dict, instruction: str) -> dict:
        self.logger.info(f"Preparing input for MolmoActLocal (instruction: '{instruction}')")
        try:
            return {
                "left_camera_rgb": obs["left_camera_rgb"],
                "front_camera_rgb": obs["front_camera_rgb"],
    ...
    def inference(self, input_dict: dict) -> dict:
        ...
        self.logger.info(
            f"Local inference completed in {time.time() - t0:.3f}s ({len(actions)} actions)"
        )
