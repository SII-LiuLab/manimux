...
Usage example:
    ...
    python -m manimux.integrations.molmoact_yam.lerobot_convert \
...
def _load_episode_from_json(ep_dir: Path, json_path: Path) -> dict[str, Any] | None:
    ...
    left_joint = np.array([json.loads(fd["left_joint"]) for fd in episode_data], dtype=np.float32)
    right_joint = np.array([json.loads(fd["right_joint"]) for fd in episode_data], dtype=np.float32)
    has_next_joint = all(
...
def _load_episode_from_h5(ep_dir: Path, h5_path: Path) -> dict[str, Any] | None:
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        next_qpos = np.asarray(f["next_state"][:], dtype=np.float32) if "next_state" in f else None
...
def infer_camera_shapes(
    episodes: list[dict[str, Any]], camera_names: list[str]
...
def sanitize_episode_metadata_for_online_viz(output_dir: str) -> None:
    ...
    if dropped_total > 0:
        print(
            "Sanitized episode metadata for online visualizer compatibility "
            f"(dropped {dropped_total} columns)."
        )
...
def create_lerobot_dataset_v30(
    ...
    features: dict[str, dict[str, Any]] = {
        ...
        "action": {
            "dtype": "float32",
            "shape": (14,),
            # Match observation.state ordering for direct visualizer comparison.
            "names": STATE_DIM_NAMES,
        },
        # Image observations
    }
...
def load_defaults_from_yaml(config_path: str) -> dict[str, Any]:
    ...
    defaults["delete_local_after_upload"] = int(
        bool(
            lerobot.get(
                "delete_local_after_upload", storage.get("delete_local_after_upload", False)
            )
        )
    )
    return defaults
...
def main():
    ...
    parser.add_argument(
        "--skip_initial_frames", type=int, default=config_defaults["skip_initial_frames"]
    )
    parser.add_argument(
        "--action_mode",
        type=str,
