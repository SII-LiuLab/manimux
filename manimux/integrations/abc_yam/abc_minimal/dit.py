...
from .config import ClipConfig, DiTConfig
...
class DiTPolicy(nn.Module):
    ...
    def sample_actions_pi_rtc(
        ...
        *,
        ...
        """Pi RTC inference-time inpainting for this reverse-time flow model.

        The paper parameterizes flow time from noise at tau=0 to actions at
        tau=1.  ABC uses the equivalent reverse convention, t=1 -> 0, and learns
        ``noise - actions``.  Therefore the clean-action estimate is
        ``x_0_hat = x_t - t * v`` and the paper's positive guidance term is
        subtracted from ABC's velocity before the negative Euler step.

        Unlike :meth:`sample_actions_rtc`, this needs no prefix-conditioned
        training.  It computes the PiGDM vector-Jacobian product at every flow
        step and supports the real-valued soft mask from Eq. 5.
        """
        if num_steps <= 0:
        ...
        model_dtype = self.y_embedder.weight.dtype
        expected_actions = (batch_size, self.chunk_length, self.action_dim)
        if noise is None:
            noise = torch.randn(
                *expected_actions,
                device=state.device,
        ...
        x_t = noise.to(device=state.device, dtype=model_dtype)
        target = action_condition.to(device=state.device, dtype=model_dtype)
        if tuple(target.shape) != expected_actions:
            raise ValueError(
                f"action_condition must have shape {expected_actions}, got {tuple(target.shape)}"
            )
        weights = condition_weights.to(device=state.device, dtype=model_dtype)
        if tuple(weights.shape) == (batch_size, self.chunk_length):
            weights = weights.unsqueeze(-1)
        expected_weights = (batch_size, self.chunk_length, 1)
        if tuple(weights.shape) != expected_weights:
            raise ValueError(
                f"condition_weights must have shape {expected_weights} or "
                f"{expected_weights[:2]}, got {tuple(weights.shape)}"
            )

        # Images and non-action conditioning are constant during denoising.
        with torch.no_grad():
            vision_tokens = self.build_vision_tokens(batch["images"]).detach()
        ...
        for i in range(num_steps):
            ...
            with torch.no_grad():
                t = torch.full(
                    (batch_size,), t_value, device=state.device, dtype=model_dtype
                )
                c = self.compute_cond(state, batch["task_vec_clip"], t).detach()
            with torch.enable_grad():
                velocity = self.predict_velocity(x_t, c, vision_tokens)
                clean_estimate = x_t - t_value * velocity
                weighted_error = (target - clean_estimate) * weights
                guidance = torch.autograd.grad(
                    outputs=clean_estimate,
                    inputs=x_t,
            ...
            # Paper Eq. 2 after substituting tau=1-t.  At t=1 the raw value
            # diverges, so clipping by beta is essential (Appendix A.2).
            tau = 1.0 - t_value
            if tau <= 0.0 or t_value <= 0.0:
                guidance_scale = float(beta)
            else:
                r_squared = (t_value * t_value) / (tau * tau + t_value * t_value)
                guidance_scale = min(float(beta), t_value / (tau * r_squared))
            guided_velocity = velocity - guidance_scale * guidance
            x_t = (x_t + guided_velocity * dt).detach()
        return x_t
