...
class EpisodeRecorder:
    ...
    def start(
        ...
        with self._lock:
            ...
            if writer_name is not None and writer_name != self.writer_name:
                ...
                self.writer = get_writer(writer_name)
    ...
    def freeze(self) -> None:
        ...
        with self._lock:
            ...
            self._pending_save = (
                trace, buf, out_dir, frame_count, task_name, include_eepose,
                self._control_timings,
            )
    ...
    def stop(self) -> Path:
        ...
        with self._lock:
            ...
            (trace, buf, out_dir, frame_count, task_name,
        ...
        try:
            ...
            if self.backend is not None:
                ...
                with (out_dir / "manimux-control.jsonl").open("w", encoding="utf-8") as handle:
                    for sample in trace:
            ...
            if include_eepose:
                try:
                    from .eepose import add_eepose_buffers
