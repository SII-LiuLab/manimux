...
Per-step state/action keys (single arm, default name "left"):
    ...
    controller-<arm>-joint latest ManiMux executor output [rad] (num_arm_joints,)
    controller-<arm>-timestamp-ns  q_cmd update time [Unix ns]  scalar
    <arm>-feedback-timestamp-ns    achieved sample time [Unix ns] scalar

Per-camera keys (unique role such as top/left/right/gemini305, img_key: rgb/left/right):
    <role>-images-<img_key>   video stream
    <role>-timestamp          per-frame capture timestamps (ms)

Optional EE pose keys use ``<arm>-`` for observed state and ``action-<arm>-``
for commanded targets: ``ee_pos``, ``ee_rotm``, ``ee_quat``,
``ee_euler_xyz``, and ``ee_transform``. Exact dimensions/conventions are stored
under ``metadata.json -> extra.eepose.fields`` in every such episode.
"""
...
def controller_joint_key(arm: str = "left") -> str:
    return f"controller-{arm}-joint"


def controller_timestamp_key(arm: str = "left") -> str:
    return f"controller-{arm}-timestamp-ns"


def feedback_timestamp_key(arm: str = "left") -> str:
    return f"{arm}-feedback-timestamp-ns"
