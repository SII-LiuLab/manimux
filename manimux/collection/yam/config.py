...
def _apply_control_profile(station: StationConfig, raw: dict) -> None:
    ...
    if runtime["executor"]["motion_limits"] is not None:
        closing_velocity = runtime["executor"]["motion_limits"]["gripper"]["max_closing_velocity"]
    ...
    for setting in ("arm_type", "gripper_force_limit", "ee_mass"):
        left = shared.get("component_hardware", {}).get("left_yam", {}).get(setting)
        right = shared.get("component_hardware", {}).get("right_yam", {}).get(setting)
        if left != right:
    ...
    for robot in station.robot.robots:
        ...
        hardware = shared.get("component_hardware", {}).get(f"{side}_yam", {})
        raw_unit = next(
