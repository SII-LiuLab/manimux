...
def create_app(
    ...
    app = FastAPI(title="ManiMux YAM Collection", version=__version__)
    session = CollectSession(cfg)
    app.state.session = session
    jobs = JobManager()

    @app.middleware("http")
    async def collection_only(request, call_next):
        from fastapi.responses import JSONResponse

        blocked = (
            request.url.path.startswith(("/api/deploy/", "/api/jobs", "/api/train/"))
            or request.url.path in {
                "/api/maintenance/reset-can", "/api/maintenance/power-off-arms",
                "/api/gpu/kill", "/api/cameras/kill-holder",
            }
        )
        if blocked:
            return JSONResponse(
                {"detail": "This copy supports Collect/Review; use ManiMux deployment tools"},
                status_code=409,
            )
    ...
    @app.get("/api/health")
    def health():
        return {"version": __version__, "task": session.cfg.task_name}
    ...
    def start_recording(body: StartRecording):
        try:
            ...
            raise HTTPException(status_code=400, detail=str(e))
        except OSError as e:  # invalid/unwritable destination
            raise HTTPException(status_code=400, detail=f"cannot create episode directory: {e}")
        return {"episode": path}
    ...
    def recording_options(body: RecordingOptions):
        ...
        if body.save_root is not None:
            ...
            if not save_root:
                raise HTTPException(status_code=400, detail="save_root cannot be empty")
            session.cfg.save_root = save_root
            if session.recorder is not None:
                session.recorder.save_root = Path(save_root).expanduser()
        if body.data_format is not None:
            if body.data_format not in FORMAT_OPTIONS:
                raise HTTPException(status_code=400, detail=f"unsupported data format {body.data_format!r}")
            session.cfg.data_format = body.data_format
    ...
    def stop_recording():
        try:
            ...
            raise HTTPException(status_code=409, detail=str(e))
        except OSError as e:  # unwritable/full save destination
            raise HTTPException(status_code=500, detail=f"episode save failed: {e}")
    ...
    def _graceful_shutdown():
        ...
        # A teaching-handle stop saves in the background so it cannot stall teleop.
        # Shutdown is the one place we deliberately wait without a timeout: a
        # completed episode must reach write_complete.flag before the process exits.
        session.wait_for_episode_save()
        for step in (session.estop, session._teardown_units, session._disconnect_cameras):
            try:
                step()
    ...
    def deploy_start(body: DeployStart):
        ...
        try:
            session.start_deploy(
                ...
                rtc=body.rtc, rtc_min_horizon=body.rtc_min_horizon,
                rtc_initial_delay_steps=body.rtc_initial_delay_steps,
                rtc_delay_buffer_size=body.rtc_delay_buffer_size,
                rtc_beta=body.rtc_beta,
                max_joint_speed=body.max_joint_speed,
                max_consecutive_clamped_steps=body.max_consecutive_clamped_steps,
            )
        except Exception as e:  # noqa: BLE001
