...
class CollectSession:
    ...
    def _connect_cameras(self, cfg: StationConfig) -> None:
        ...
        for c in cameras:
            ...
            try:
                ...
                worker = None
                # Scope driver startup and cleanup to this one camera.
                driver, = build_cameras_from_config(replace(cfg, cameras=[c]))
    ...
    def go_live(self, cfg: StationConfig, followers_only: bool = False) -> None:
        ...
        try:
            self.cfg = cfg
            self.units = build_arm_units(cfg, followers_only=followers_only)
    ...
    def collection_timing_error(self) -> str | None:
        ...
        if self.units:
            backend = self.units[0].robot.backend
            if backend.config["executor"]["type"] not in {"direct", "smooth"}:
    ...
    def set_collection_hz(self, hz: float) -> None:
        ...
        with ExitStack() as locks:
            ...
            if self.units:
                ...
                config = load_backend_config(proposed)
    ...
    def enable_sync(self) -> None:
        with self._sync_lock:
            ...
            # Preserve the existing i2rt slow_move alignment pattern.
    ...
    def start_recording(
        ...
        with self._recording_lock:
            return self._start_recording(
                task_name, include_eepose, save_root, data_format,
            )

    def _start_recording(
        self, task_name, include_eepose, save_root, data_format,
    ) -> str:
