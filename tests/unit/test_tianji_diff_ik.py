...
from manimux.kinematics.base import ArmKinematicsBase
...
def diff_config():
    ...
    config["policy"]["options"].update(
        ik_backend="diff",
        diff_ik=yaml.safe_load(
            (ROOT / "configs/policy/umi_dp/adapter/tianji_diff.yaml").read_text()
        ),
    )
...
@pytest.fixture
def solver():
    kin = TianjiArmKinematics()
    config = diff_config()
    parameters = dict(config["policy"]["options"]["diff_ik"])
    parameters["lag_policy"] = "abort"
    return TianjiDifferentialKinematics(kin, DifferentialIKConfig(**parameters))


def test_diff_and_arm_implement_shared_kinematics_contracts(solver):
    assert isinstance(solver, ArmKinematicsBase)
    assert isinstance(solver.kinematics, ArmKinematicsBase)
...
def test_rate_bound_tracking_lag_and_dt_cap(solver, dt):
    target = solver.kinematics.flange(START)
    target[:3, 3] += [0.2, -0.1, 0.15]
    result = solver.ik(target, START, duration_s=dt)
    assert not result.ok and result.reason == "tracking_lag"
    assert result.diagnostics["pos_err_mm"] > solver.config.max_lag_mm
    delta = np.max(np.abs(result.diagnostics["qdot_rad_s"] * min(dt, solver.config.dt_max_s)))
    assert 0 < delta <= solver.config.max_velocity_rad_s * min(dt, solver.config.dt_max_s) + 1e-14
    assert (
        np.max(np.abs(result.diagnostics["qdot_rad_s"]))
        <= solver.config.max_velocity_rad_s + 1e-14
    )


def test_reset_drops_dual_state(solver):
    target = solver.kinematics.flange(START + np.radians([0.08] * 7))
    first = solver.ik(target, START, duration_s=0.004)
    assert first.ok and first.diagnostics["pos_err_mm"] < 0.05
    old_problem = solver._problem
    solver.reset()
    assert solver._problem is None
    assert not solver._previous_velocity.any()
    second = solver.ik(target, START, duration_s=0.004)
...
def test_empty_box_does_not_reuse_previous_osqp_problem(solver):
    assert solver.ik(solver.kinematics.flange(START), START, duration_s=0.004).ok
    problem = solver._problem
    bad = START.copy()
    bad[0] = np.radians(170)
    result = solver.ik(solver.kinematics.flange(bad), bad, duration_s=0.004)
    assert not result.ok and result.reason == "qp_infeasible"
    assert result.diagnostics["detail"]["note"] == "empty qdot box"
...
def test_j67_constraint_conflict_is_rejected_in_every_quadrant(solver, j6, j7):
    ...
    result = solver.ik(solver.kinematics.flange(joints), joints, duration_s=0.004)
    assert not result.ok and result.reason == "qp_infeasible"
    assert "osqp_status" in result.diagnostics["detail"]
...
def test_optional_orientation_lag_guard(solver):
    ...
    result = solver.ik(target, START, duration_s=0.004)
    assert not result.ok and result.reason == "tracking_lag"
    assert result.diagnostics["rot_err_deg"] > solver.config.max_lag_deg


def test_report_lag_policy_keeps_the_bounded_step(solver):
    target = solver.kinematics.flange(START)
    target[:3, 3] += [0.2, -0.1, 0.15]
    aborted = solver.ik(target, START, duration_s=0.004)
    solver.reset()
    solver.config.lag_policy = "report"
    reported = solver.ik(target, START, duration_s=0.004)
    assert (
        not aborted.ok
        and aborted.reason == "tracking_lag"
        and aborted.diagnostics["lag_exceeded"]
    )
    assert reported.ok and reported.reason == "ok" and reported.diagnostics["lag_exceeded"]
    assert not reported.target_reached
    expected = START + aborted.diagnostics["qdot_rad_s"] * 0.004
    np.testing.assert_allclose(reported.joints, expected, atol=5e-16, rtol=0)
...
def test_invalid_input_never_reaches_osqp(solver, kind):
    joints = START.copy()
    target = solver.kinematics.flange(joints)
    dt = 0.004
    if kind == "joints":
        joints[0] = np.nan
    elif kind == "target":
        ...
        dt = 0
    assert solver.ik(target, joints, duration_s=dt).reason == "invalid_input"
    assert solver._problem is None


def test_nonfinite_qp_solution_rejected(solver, monkeypatch):
    target = solver.kinematics.flange(START)
    assert solver.ik(target, START, duration_s=0.004).ok
    monkeypatch.setattr(
        solver._problem,
        "solve",
        lambda **kwargs: SimpleNamespace(info=SimpleNamespace(status_val=1), x=np.full(7, np.nan)),
    )
    assert solver.ik(target, START, duration_s=0.004).reason == "fk_mismatch"
...
def test_cannot_relax_embodiment_margin_or_disable_umi_interference(solver):
    ...
    with pytest.raises(ValueError, match="cannot reduce"):
        TianjiDifferentialKinematics(solver.kinematics, config)
...
def test_real_diff_adapter_chunk_timing_and_atomic_rejection(horizon, offset):
    ...
    config["policy"]["options"].update(first_action_offset_s=offset)
    identity = config["policy"]["expected_backend"]["model"]
    identity["action_horizon"] = horizon
    identity["first_action_offset_s"] = offset
    config["policy"]["options"]["diff_ik"]["lag_policy"] = "abort"
    ...
    frames = {
        ...
        for name in config["policy"]["options"]["camera_map"].values()
    }
    ...
    for index in range(horizon):
        ...
        for side in ("left", "right"):
            action[side + "_ee_pose"] = matrix_pose(
                adapter.kin[side].fk(np.r_[target_joints, 0.8])
            )
            action[side + "_ee_joint_state"] = np.array([0.8])
        actions.append(action)
    adapter.prepare_request(InferenceRequest("test", 1, 10**9, 2 * 10**9, window))
    context = ActionContext(1, 10**9, 10**9, measured_state=state)
    chunk = adapter.decode_action({"actions": actions}, context)
    ...
    for side in ("left", "right"):
        assert adapter.kin[side].arm.kinematics._sdk is None
    ...
    assert (
        lag["right"]["worst_lag_mm"]
        > reporting.kin["right"].arm.config.max_lag_mm
    )
