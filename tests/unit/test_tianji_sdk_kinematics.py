...
def test_sdk_units_arm_selection_and_result(sdk, arm, index):
    ...
    assert isinstance(kin, ArmKinematicsBase)
    assert kin.num_joints == 7 and sdk.arm == index
...
def test_rejected_ik_never_returns_commandable_joints(sdk, outcome, reason):
    ...
    sdk.outcome = outcome
    result = kin.ik(kin.fk(np.zeros(7)), np.zeros(7))
...
def test_success_flag_is_not_enough_without_fk_validation(sdk, kind):
    kin = TianjiSDKKinematics()
    target = kin.fk(np.zeros(7))
    if kind == "position":
        target[0, 3] += 0.01
    else:
        target[:3, :3] = [[0, -1, 0], [1, 0, 0], [0, 0, 1]]
    result = kin.ik(target, np.zeros(7))
...
def test_invalid_pose_inputs(sdk, target):
    with pytest.raises(ValueError):
        TianjiSDKKinematics().ik(target, np.zeros(7))
...
def test_native_fk_and_ik_round_trip(arm):
    kin = TianjiSDKKinematics(arm)
    np.testing.assert_allclose(kin.fk(np.zeros(7))[:3, 3], [0, 0, 0.8705], atol=1e-12)
    ...
    for seed in (q, q + np.radians([0.1, 0, 0, 0, 0, 0, 0])):
        result = kin.ik(target, seed)
        assert result.converged, result.reason
        np.testing.assert_allclose(kin.fk(result.joints), target, atol=1e-7)
    target[0, 3] = 10
    result = kin.ik(target, q)
...
def test_native_instances_restore_model_and_remove_tool_offsets(tmp_path):
    ...
    np.testing.assert_allclose(original.fk(q), baseline, atol=1e-12)
    np.testing.assert_allclose(other.fk(q), shifted, atol=1e-12)
    tool = np.eye(4)
