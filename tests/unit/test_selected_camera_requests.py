...
from manimux.server.sensor.taccap.client import CameraClient, CameraClientError, CameraSubscriber
from manimux.server.sensor.taccap.server import CameraServer, _build_cameras_from_config
...
def test_service_round_trip_preserves_rgb_and_timestamped_umi_frames(running_camera_service):
    ...
    sensor = TimestampedCameraSensor(
        {"options": {
            ...
            "output_names": {
                "left_wrist": "left_wrist_camera", "right_wrist": "right_wrist_camera",
            },
...
def test_duplicate_service_exits_before_camera_access(running_camera_service):
    ...
    result = subprocess.run(
        [sys.executable, "-B", "-m", "manimux.server.sensor.taccap",
         "--experiment", "configs/experiments/pass_ball/tianji_taccap_umi_dp.yaml",
         "--local", "configs/local/tianji_taccap.example.yaml",
         "--rep-endpoint", running_camera_service["rep"],
         "--pub-endpoint", running_camera_service["pub"]],
...
def test_camera_factory_builds_taccap_only(monkeypatch, tmp_path):
    def open_camera(serial, **options):
        return SimpleNamespace(serial=serial, options=options, close=lambda: None)

    monkeypatch.setitem(
        sys.modules,
        "manimux.embodiments.sensor.taccap",
        SimpleNamespace(TacCapCamera=open_camera),
    )
    specs = {"wrist": {"type": "taccap", "camera_serial": "taccap-serial"}}
    ...
    assert cameras["wrist"].serial == "taccap-serial"
    assert cameras["wrist"].options["fps"] == 30
