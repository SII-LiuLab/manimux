...
class PartitionAdapter(FakePolicyAdapter):
    ...
    def decode_hold_partition(self, raw, context, partition, reason):
        ...
        return ActionChunk(
            ...
            hold_from_step={partition: 0},
            metadata={"holds": {partition: reason}},
        )
    ...
    try:
        ...
        def decode(seq):
            ...
            context = ActionContext(
                ...
                now,
                now,
                measured_state=RobotState(
                    {name: np.zeros(2) for name in PartitionAdapter.decode_partitions}, now, 0
                ),
                independent_groups=True,
                decode_budget_ms=40,
            )
            decoder.submit(
                InferenceResponse("test", seq, now, 0, None, observation_time_ns=now),
                context,
                now + 3_000_000_000,
            )
            result = None
        ...
        assert duration < 0.3  # Does not wait for the 400ms right-arm job.
