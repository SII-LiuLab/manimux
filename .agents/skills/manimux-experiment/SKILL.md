...
Read the selected `server` and `infra` configs and the infra's `control_profile`.
...
```
```bash
XPolicyLab/policy/Pi_05/openpi/.venv/bin/python scripts/servers/pi05_yam_server.py \
...
- Viewer flow is Prepare → Start rollout → Finish & Home. `src/manimux/session.py`
  waits for Prepare before creating a rollout; `src/manimux/runtime/edge.py` connects
...
- For collection, use the body's own entry point. YAM uses
  `python -m manimux.collection --config configs/collection/yam/station.yaml` from its
